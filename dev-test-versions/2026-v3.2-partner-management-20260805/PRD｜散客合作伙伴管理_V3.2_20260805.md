...
flowchart TD
    A["后台维护合作伙伴档案"] --> B{"合作类型"}
    B -->|"企业 / 个人"| C["同一事务建立挂帐、预付款两个子账户"]
    B -->|"公共临时客户"| D["系统唯一公共临时客户\n仅建立挂帐子账户、固定启用"]
    C --> E["配置账户支付方式、账户状态及额度限制"]
    D --> F["配置挂帐额度限制\n公共临时客户支付方式固定为挂帐"]
    E --> G["后台充值申请"]
    F --> G
    G --> H{"充值对象"}
    H -->|"企业 / 个人"| I{"充值类型"}
    I -->|"挂帐 / 预付款"| I1["金额大于 0，写入当前账户支付方式对应子账户，生成充值记录和充值流水"]
    I -->|"调账"| I2["金额可正可负且不为 0，写入当前配置子账户及调账流水"]
    I -->|"其他收入"| I3["金额大于 0，不改变任何账户，生成独立其他收入记录和流水"]
    H -->|"公共临时客户"| J{"挂帐回款或调账"}
    ...
    D --> K
    K --> L{"选择对象"}
    L -->|"企业 / 个人"| M["选择启用固定合作伙伴，加载负责人、联系人及账户支付方式"]
    ...
    AI -->|"返回修改"| AF
    AI -->|"确认继续"| AH
    AH -->|"新建企业 / 个人"| AJ["创建长期合作伙伴及双子账户"]
    AH -->|"选择已有伙伴"| AK["仅可选择启用的企业 / 个人"]
    AJ --> AL["同一事务迁移"]
    AK --> AL
    AL --> AM["订单和关联售后单改为目标伙伴当前归属"]
    AL --> AN["公共临时客户原消费、退款流水保留不改"]
    AL --> AO["目标伙伴按当前账户支付方式重建消费负数、退款正数流水"]
    AL --> AP["公共临时客户生成正数调账-往来充值及调账流水"]
    ...
    AT --> BA
    BA --> BB{"订单当前归属及回款状态"}
    BB -->|"企业 / 个人或已迁移订单"| BC["按当前订单支付账户增加退款金额，生成退款正数流水"]
...
| 合作类型 | 定义 | 账户结构 |
| --- | --- | --- |
| 企业 | 固定合作伙伴 | 挂帐、预付款两个子账户 |
| 个人 | 固定合作伙伴 | 挂帐、预付款两个子账户 |
| 临时客户 | 系统唯一的公共临时客户，名称固定为“公共临时客户” | 仅挂帐子账户 |

### 4.2 账户结构

| 对象 | 挂帐账户 | 预付款账户 | 账户支付方式 |
| --- | --- | --- | --- |
| 企业、个人 | 自动创建，可正可负 | 自动创建，可正可负 | 配置为“挂帐”或“预付款” |
| 公共临时客户 | 自动创建，可正可负 | 不创建 | 固定为“挂帐” |

- 账户号由系统生成；新增前显示“—”，创建成功后展示真实账户号且不可修改。
- 企业、个人创建后即同时存在两个子账户；切换账户支付方式不新增、删除、合并或转移子账户。
...
- 固定合作伙伴同时拥有挂帐和预付款子账户；具体由账户支付方式决定售票和普通充值当前使用哪个账户。
...
- 临时客户：联系人、联系电话、对接负责人、账户支付方式、状态和账户号不展示、不填写；系统固定账户支付方式为挂帐、状态为启用。系统已经存在公共临时客户时，不允许再次创建。
...
- 保存后仅影响后续售票及后续普通充值的当前使用账户。

#### 保存逻辑

1. 校验必填、长度、联系人与联系电话、负责人及账户支付方式规则。
2. 对文本字段去除首尾空格，并校验合作伙伴名称唯一性。
3. 类型为临时客户时，校验系统中不存在其他公共临时客户。
4. 创建时在同一业务事务中创建合作伙伴档案、主账户和子账户；企业、个人创建挂帐和预付款子账户，临时客户只创建挂帐子账户。
5. 任一步骤失败均不保留本次新增数据；成功后记录更新人和更新时间。
6. 编辑不修改账户号和既有子账户结构；历史订单、充值和资金流水不因编辑回写。
...
- 标题“编辑合作伙伴”。
- 合作类型、账户号只读。
- 其他字段按本节规则编辑；保存后更新更新人、更新时间，不重新生成账户或子账户。
...
- 跳转合作伙伴账户管理，携带账户号与当前账户支付方式。
- 目标页只加载该合作伙伴；默认打开当前账户支付方式对应的挂帐/预付款标签。

#### 删除

- 任一子账户已经产生充值、消费或退款记录时不允许删除；余额恢复到 0 也不例外。
...
- 从合作伙伴列表点击“账户”进入时，携带账户号与账户支付方式，打开对应标签、回填账户号并自动查询。
- 每次切换标签均清空筛选条件并查询当前标签全部账户；用户可修改、清除或重置筛选条件。
- 标签参数缺失或无法识别时默认打开挂帐管理；账户号不存在或合作伙伴已删除时，保留账户号条件并显示暂无数据；在预付款管理查询临时客户账户号时显示暂无数据。
- 企业、个人的挂帐、预付款共用一个对外账户号，系统内部用“账户号＋子账户类型”区分；临时客户只出现在挂帐管理。

### 6.2 挂帐管理

#### 筛选条件

| 账户类型 | 合作伙伴名称 | 账户号 | 状态 | 额度限制 | 挂帐金额区间 |
| --- | --- | --- | --- | --- | --- |
| 企业、个人、临时客户 | 模糊搜索 | 精确/模糊搜索 | 启用、禁用 | 启用、禁用 | 起止金额 |

#### 列表字段

| 账户类型 | 合作伙伴名称 | 账户号 | 挂帐金额 | 挂帐额度 | 可用挂帐额度 | 额度限制 | 状态 | 操作 |
| --- | --- | --- | --- | --- | --- | --- | --- |
...
- 点击“额度记录”打开只读弹窗，顶部展示合作伙伴名称、账户类型、账户号、当前挂帐金额、当前挂帐额度、当前额度限制状态。
...
| 账户类型 | 合作伙伴名称 | 账户号 | 状态 | 额度限制 | 预付款余额区间 |
| --- | --- | --- | --- | --- | --- |
| 企业、个人 | 模糊搜索 | 精确/模糊搜索 | 启用、禁用 | 启用、禁用 | 起止金额 |

#### 列表字段

| 账户类型 | 合作伙伴名称 | 账户号 | 预付款余额 | 额度限制 | 状态 | 操作 |
| --- | --- | --- | --- | --- | --- |
...
- “充值记录”跳转充值申请，自动带入当前合作伙伴，默认打开“账户充值”。
- “资金流水”跳转资金流水，自动带入当前合作伙伴，默认标签为“全部”。
...
- 新增时所有字段为空；选择合作伙伴后再加载账户号、当前账户信息和可选充值类型。
...
| 第一行 | 第二行（默认） | 展开后 |
| --- | --- | --- |
| 合作伙伴名称、账户号、充值记录单号 | 充值类型、充值状态、查询/重置/展开 | 对接负责人、申请人、支付时间（日期）、支付方式、柜台号/POS编号 |
...
| 充值记录单号 | 账户类型 | 合作伙伴名称 | 账户号 | 充值类型 | 充值金额 | 申请人 | 对接负责人 | 支付方式 | 柜台号/POS编号 | 支付时间 | 申请时间 | 充值状态 | 完成时间 | 操作 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 可查看、复制 | 企业/个人/临时客户 | 名称 | 账户号 | 挂帐/预付款/调账/调账-往来 | 挂帐、预付款为正数；调账可正可负；调账-往来为系统正数 | 操作人员快照 | 员工名＋手机号快照 | 实际收款方式；调账、调账-往来为“－” | 售票窗口收款方式为扫码付或银行卡时保存输入值；其余为“－” | 后台手工充值取所选日期 `00:00:00`；售票窗口充值取提交时间 | 实际提交时间（秒级） | 充值成功 | 实际处理完成时间（秒级） | 查看 |

`其他收入`单独在“其他收入”标签展示，不与账户充值列表混合。

`调账-往来`仅作为列表筛选和展示类型，不出现在人工新增充值的类型下拉中。
...
选择企业或个人合作伙伴后，字段按当前“账户支付方式”加载。

| 字段 | 普通挂帐/预付款 | 调账 | 其他收入 |
| --- | --- | --- | --- |
| 合作伙伴 | 必填，搜索选择 | 必填，搜索选择 | 必填，搜索选择 |
| 充值账号 | 展示当前合作伙伴账户号 | 展示当前合作伙伴账户号 | 不展示账户影响信息 |
| 充值类型 | 当前配置账户类型、调账、其他收入三选一 | 同左 | 同左 |
| 当前账户金额 | 挂帐显示“当前挂帐金额”；预付款显示“当前预付款余额” | 显示当前配置账户金额 | 不展示 |
| 充值金额 | 必填，必须大于 0 | 必填，可正可负，且不可为 0 | 必填，必须大于 0 |
| 支付时间 | 必填，日期选择；系统保存为所选日期 `00:00:00` | 不展示、不保存 | 必填，日期选择；系统保存为所选日期 `00:00:00` |
| 客户/付款人 | 选填 | 不展示、不保存 | 选填 |
| 支付方式 | 必填：现金/银行转账 | 不展示、不保存 | 必填：现金/银行转账 |
| 柜台号/POS编号 | 不展示、不保存 | 不展示、不保存 | 不展示、不保存 |
| 备注说明 | 选填，最多 500 字 | 选填，最多 500 字 | 选填，最多 500 字 |
| 充值相关资料 | 选填；使用系统通用附件上传组件，资料随充值记录保存 | 选填；使用系统通用附件上传组件，资料随充值记录保存 | 选填；使用系统通用附件上传组件，资料随充值记录保存 |
...
- 不允许从普通充值表单直接选择另一类账户。
- 普通挂帐、预付款及其他收入的充值金额均必须大于 0；账户金额允许正负数，是由消费、退款及调账等业务动作形成。仅调账允许录入正数或负数，且不可为 0。
- 调账作用于当前账户支付方式对应的子账户。
- `其他收入`只记录该合作伙伴产生的收款，不改变挂帐或预付款账户金额。
- 备注提示：`如为银行转账，请备注收款银行账号尾号后4位。`
- 调账不填写支付时间、客户/付款人、支付方式，仅填写金额、备注说明及附件；备注不强制填写。

### 7.4 公共临时客户：账户充值表单

公共临时客户仅有挂帐账户；可选充值类型仅为：**挂帐、调账**。
...
| 充值记录单号 | 合作伙伴名称 | 充值类型 | 充值金额 | 申请人 | 对接负责人 | 支付方式 | 柜台号/POS编号 | 支付时间 | 申请时间 | 充值状态 | 完成时间 | 操作 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 单独生成 | 名称 | 其他收入 | 正数 | 操作人员 | 当时负责人快照 | 实际收款方式 | 售票窗口扫码付或银行卡收款时保存输入值；其余为“－” | 实际支付时间 | 提交时间 | 充值成功 | 成功时间 | 查看 |

- 不展示账户号，不改变合作伙伴的挂帐或预付款账户数据。
- 非公共临时客户可手工发起其他收入；公共临时客户仅由挂帐回款的剩余金额自动生成。
...
第二步默认创建新的企业或个人长期合作伙伴，也可选择已有启用的企业、个人合作伙伴。确认后在同一业务事务内迁移订单和关联售后单归属：公共临时客户的历史消费、退款流水保留；目标合作伙伴按当前账户支付方式重建相同订单、售后单的消费/退款流水；公共临时客户自动生成正数“调账-往来”充值与同额调账流水。金额为已选订单总额减关联售后退款总额。失败时全部回滚；详情、字段、审计和后续回款规则见“临时客户转合作伙伴详细规则”附录。
...
- 导入模板：[合作伙伴充值批量导入模板.xlsx](../../../partner-management-finance-adjustment/合作伙伴充值批量导入模板.xlsx)。
...
| 账户号 | 是 | 必须与合作伙伴一致 |
| 充值类型 | 是 | 挂帐、预付款、调账、其他收入 |
| 充值金额 | 是 | 挂帐、预付款、其他收入必须大于 0；调账可正可负且不得为 0 |
| 支付时间 | 挂帐/预付款/其他收入必填；调账可空 | 填写日期，导入后统一转为该日期 `00:00:00` |
| 客户/付款人 | 否 | 调账填写时不保存 |
| 支付方式 | 挂帐/预付款/其他收入必填 | 现金、银行转账；调账填写时不保存 |
...
| 导入充值类型 | 校验与处理 |
| --- | --- |
| 挂帐 | 仅当前账户支付方式为挂帐的合作伙伴允许导入；写入挂帐账户 |
| 预付款 | 仅当前账户支付方式为预付款的合作伙伴允许导入；写入预付款账户 |
| 调账 | 写入当前账户支付方式对应的子账户；可正可负 |
| 其他收入 | 不影响账户；金额必须大于 0；不受账户支付方式限制 |

- 导入不自动修改合作伙伴账户支付方式。
- 导入成功后自动生成充值记录单号，自动匹配账户类型，状态为“充值成功”。
...
| 类型为空 | 充值类型不能为空 |
| 类型非法 | 充值类型不正确 |
| 类型与账户支付方式冲突 | 充值类型与账户支付方式不一致 |
| 金额非法 | 充值金额格式错误 |
| 金额为 0 | 充值金额不能为0 |
| 其他收入金额不为正 | 其他收入金额须大于0 |
| 支付时间缺失 | 支付时间不能为空 |
| 支付时间非法 | 支付时间格式错误 |
| 支付方式非法或缺失 | 支付方式不正确 |
| 系统异常 | 导入失败，请重试 |
...
| 第一行 | 第二行 | 展开后的第三行 |
| --- | --- | --- |
| 合作伙伴类型、合作伙伴名称、账户号 | 账户类型、发生时间、查询、重置、展开/收起（最右侧） | 对接负责人、单据编号 |
...
| 合作伙伴类型 | 合作伙伴名称 | 账户号 | 处理操作 | 账户类型 | 金额（元） | 单据编号 | 对接负责人 | 发生时间 | 支付方式 | 柜台号/POS编号 | 消费回款 | 备注 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
- 调账：包含人工调账、系统自动生成的“已回款订单退款调账”、系统自动生成的“调账-往来”。人工调账的处理操作显示“调账”；自动调账与调账-往来均按其实际账户变动展示。`调账-往来`关联的充值记录单号可查看、复制。
...
- 本月到款：充值类型为“挂帐、预付款”的金额，按充值支付时间计入；不包含其他收入、调账、调账-往来。
- 调账：在明细末尾独立“调账”行汇总所选范围内“调账、调账-往来”。正数计入本月到款，负数计入本月消费，时间均取充值支付时间。
...
右侧合作伙伴信息按所选对象及其**账户支付方式**展示。更换合作伙伴或切换为临时客户时，仅刷新本信息区；已选择的票、数量、游客信息和订单计算结果均不刷新。

| 对象与账户支付方式 | 右侧展示字段 |
| --- | --- |
| 企业/个人，账户支付方式为挂帐 | 合作伙伴、对接负责人、联系人、挂帐金额、可用挂帐额度 |
| 企业/个人，账户支付方式为预付款 | 合作伙伴、对接负责人、联系人、预付款余额、额度限制（启用/禁用） |
| 公共临时客户 | 合作伙伴：公共临时客户、挂帐金额、可用挂帐额度、客户名称、订单对接负责人 |

- 支付方式直接加载合作伙伴配置的**账户支付方式**，在主页面和支付订单确认弹窗均以锁定的下拉外观展示，不允许切换。
- 账户支付方式为挂帐时，订单消费作用于挂帐账户；为预付款时，订单消费作用于预付款账户。
...
企业、个人客户如需先付款，收银员在售票主页面点击“充值”，完成独立账户充值后，再按该合作伙伴账户支付方式正常下单。
...
| 账户支付方式 | 只读 | 展示挂帐或预付款；决定本次账户充值的充值类型。 |
| 收款方式 | 单选 | 现金、扫码付、银行卡、微信、支付宝；必填。 |
| 客户/付款人 | 文本输入 | 非必填，最多 50 字；保存至本次账户充值记录的客户/付款人字段。 |
| 充值金额 | 数字输入 | 充值到当前账户支付方式对应子账户；最多两位小数，可填 `0.00`。 |
...
充值金额大于 `0.00` 时，生成一条与后台“账户充值”规则一致的充值记录，并增加当前账户支付方式对应子账户；账户支付方式为挂帐时充值类型为“挂帐”，为预付款时充值类型为“预付款”。

| 充值记录字段 | 保存规则 | 充值详情展示 |
| --- | --- | --- |
| 合作伙伴 | 取售票窗口已选合作伙伴 | 展示 |
| 充值类型 | 取合作伙伴当前账户支付方式：挂帐或预付款 | 展示 |
...
点击主页面“确认下单”仅打开“支付订单确认”弹窗，不创建订单、不调整账户。弹窗沿用售票窗口现有的门票表格、总数、“一票多人 / 一人一票”互斥单选和“取消出票 / 确认出票”按钮；支付方式加载当前合作伙伴账户支付方式，锁定且不可切换。
...
- 订单支付信息中的支付类型展示确认出票时实际使用的账户支付方式。
...
筛选条件按“合作伙伴名称、充值类型、支付方式、柜台号/POS编号、对接负责人、支付时间、申请人、充值记录单号”顺序展示。默认展示两行，其余条件折叠；支付时间为日期范围，默认当天；申请人默认当前售票员且允许修改。

列表字段为：充值记录单号、账户类型、合作伙伴名称、账户号、充值类型、充值金额、申请人、对接负责人、支付方式、柜台号/POS编号、支付时间、充值状态、完成时间、操作。页面不提供顶部业务操作按钮，行内仅“查看”；详情中柜台号/POS编号紧随支付方式展示，非扫码付、银行卡记录展示“－”。

#### 11.7.3 其他收入记录

筛选条件按“合作伙伴名称、支付方式、柜台号/POS编号、对接负责人、支付时间、申请人、充值记录单号”顺序展示。默认展示两行，其余条件折叠；支付时间为日期范围，默认当天；申请人默认当前售票员且允许修改。

列表字段为：充值记录单号、合作伙伴名称、充值类型、充值金额、申请人、对接负责人、支付方式、柜台号/POS编号、支付时间、充值状态、完成时间、操作。页面不提供顶部业务操作按钮，行内仅“查看”；详情中柜台号/POS编号紧随支付方式展示，非扫码付、银行卡记录展示“－”。
...
报表标题为“合作伙伴充值统计”，标题下展示“开始日期 {开始日期} 结束日期 {结束日期}”。统计按“类型、合作伙伴名称、对接负责人快照、充值类型”汇总；同一合作伙伴在统计期间内对接负责人快照不同，必须拆分为不同统计行。相同“类型、合作伙伴名称、对接负责人快照”的连续明细行合并展示前三列；该组存在多条充值类型汇总行时，明细后增加一行“合作伙伴小计”，汇总该组全部充值金额及各支付方式金额。

列表列为：类型、合作伙伴名称、对接负责人、充值类型、充值金额、支付方式（现金、支付宝、微信、银行卡、扫码付）。银行卡在“支付方式”下按已保存的 POS 编号继续拆分列，扫码付按已保存的柜台号继续拆分列，列名分别取对应编号；现金、支付宝、微信各自汇总为一列。充值金额为组内记录金额合计；各支付方式列按相同分组及支付方式汇总，未发生时显示 `0.00`。表格最末增加“总计”行，汇总全部查询结果的充值金额和各支付方式金额；表格下方展示制单人“当前售票员”及本次点击查询时的打印时间。统计表使用完整横竖边框，除报表主标题居中外，表头及全部数据单元格左对齐。
...
- 表单复用合作伙伴新增字段与校验：合作类型、合作伙伴名称、联系人、联系电话、对接负责人、账户支付方式、备注。
- 合作类型仅可选企业、个人；不得创建临时客户。
- 状态固定为启用，页面不展示状态修改入口；账户号创建成功后系统生成。
- 确认后先创建合作伙伴及其挂帐、预付款两个子账户，再执行本次订单迁移；任一后续迁移步骤失败则新建合作伙伴及账户一并回滚。

#### 选择已有合作伙伴

- 选择已有合作伙伴时，仅展示状态为“启用”的企业、个人合作伙伴；禁用合作伙伴不进入选择列表，也不可作为迁移目标。
- 订单迁移后使用目标合作伙伴当前配置的账户支付方式对应子账户。
...
4. 目标合作伙伴重建流水与账户处理：以原订单、售后单为同一关联单据，在目标合作伙伴当前账户支付方式对应子账户中重新生成消费负数流水、退款正数流水；金额、订单号/售后单号保持一致。每条重建流水备注固定为`临时客户转合作伙伴生成，来源公共临时客户，转移单号：{转移单号}`。目标账户按该组重建流水的净额扣减，净额为每笔订单的`本次应回款金额 = 订单总金额 − 已退款金额`之和。
5. 公共临时客户账户处理：公共临时客户原账户已因历史消费和退款发生金额变化。为结清原临时客户往来，自动生成一笔正数的系统充值记录及同额“调账”资金流水，充值类型为`调账-往来`，金额为已选订单本次应回款金额合计；公共临时客户挂帐账户同步增加相同金额。

`调账-往来`是系统自动处理类型，归属于充值记录：不出现在人工新增的充值类型下拉选项中，不允许人工创建或编辑；但在充值列表、充值详情、资金流水“调账”标签与后续凭证处理中正常展示和追溯。资金流水处理操作显示“调账”，账户类型显示“挂帐”。
...
| 变更账户支付方式 | 源账户不为 0 时拦截；不生成资金记录；为 0 时允许保存 |
| 企业/个人普通充值 | 仅出现当前账户类型、调账、其他收入；无订单平账区 |
...
| 售票窗口固定合作伙伴下单 | 支付方式锁定为账户支付方式；确认出票生成订单、账户消费和消费流水 |
...
| 批量导入 | 合法行导入成功，非法行逐条反馈；不全量回滚；不自动切换账户支付方式 |
