(() => {
  ...
  const refreshConfigSaveState = () => { const state=q('#configSaveState'); if(state) state.textContent=configSaved?'配置已保存':'配置未保存'; };
  const markDirty = () => { stepDirty=true; configSaved=false; refreshConfigSaveState(); };
  ...
  function requestLeave(action, validateCurrentStep=false){
    if(stepDirty&&!saveCurrent(true,validateCurrentStep))return false;
    action();
    return true;
  }
  ...
  const projectSetupFields = (prefix,session=null) => {
    ...
    const rows=projects.map(project=>{const checked=associated.includes(project.id);const key=session?sessionProjectKey(window.editingSessionDate,session,project.id):'';const booked=key?(config.projectSessionBooked[key]||0):0;const quota=key?config.projectSessionQuotas[key]:undefined;const historicalDisabled=!activeProjects.includes(project)&&checked;const associationLocked=historicalDisabled||(checked&&booked>0);const state=historicalDisabled?'已禁用，保留历史关联':booked?`已预约 ${booked} 人`:'暂无预约';return `<div class="session-project-select-row"><label title="${project.name}"><input type="checkbox" data-project-association="${prefix}" value="${project.id}" ${checked?'checked':''} ${associationLocked?'disabled':''}><b>${project.name}</b></label><small>${state}</small><input class="project-setup-quota" data-project-setup-quota="${project.id}" type="number" min="${Math.max(1,booked)}" inputmode="numeric" value="${quota??''}" placeholder="必填名额" ${historicalDisabled?'disabled':''}></div>`;}).join('');
  ...
  const views = [
    ...
    () => `${heading(4,'控制游客端剩余名额以及已截止场次的展示方式。')}
      <section class="config-card"><h3>游客端场次展示</h3>${switchRow('showQuota','向游客展示剩余名额','开启后日期、场次和项目显示“剩余 X 名”或“不限额”；关闭后仅显示“可预约”')}${switchRow('hideExpired','隐藏已截止场次','开启后游客端不展示已截止场次；关闭后展示并标记“已截止”')}<p class="selection-rule-tip"><i>i</i>每次预约固定选择 1 个场次</p></section>`,
    ...
    () => `${heading(6,'保存当前配置，必要时同步发布到正式游客端。')}
      <section class="config-card publish-status-card"><div class="publish-status-line"><span>当前活动状态</span><b class="${currentActivity?.status==='published'?'is-published':'is-offline'}">${currentActivity?.status==='published'?'已上架':'已下架'}</b></div><p>${configSaved?'当前配置已保存。':'当前配置未保存；顶部切换和游客端效果仅用于暂存与预览。'} ${currentActivity?.status==='published'?'正式游客端正在使用已发布版本。':'当前不接受新预约。'}</p></section>
      <section class="config-card"><div class="publish-page-head"><div><h3>发布检查</h3><p>全部必备项通过后才允许保存并发布</p></div><span>${publishChecks().filter(item=>item[1]).length}/${publishChecks().length}</span></div><div class="publish-check detailed">${publishChecks().map(([title,passed,note])=>`<div class="${passed?'':'missing'}"><i>${passed?'✓':'!'}</i><span><b>${title}</b><small>${note}</small></span></div>`).join('')}</div></section>
      <section class="config-card publish-action-card"><h3>发布更新</h3><p class="publish-summary">先保存当前配置，再将最新配置更新到线上</p><button class="publish-button ${publishChecks().some(item=>!item[1])?'is-blocked':''}" id="savePublishConfig" aria-disabled="${publishChecks().some(item=>!item[1])}">保存并发布</button>${currentActivity?.status==='published'?'<button class="publish-button offline" id="offlineConfig">下架活动</button>':'<button class="publish-button secondary" id="saveConfig">保存</button><button class="publish-button offline" id="deleteConfig">删除活动</button>'}</section>`
  ];
  ...
  function renderConfig(preserveScroll=false) {
    ...
    q('#configNext').hidden=stepIndex===steps.length-1;
  ...
  window.openConfig = (activity=activities[0], isNew=false, returnPage='activities') => {
    ...
    configSaved=saved?.configSaved ?? !isNew;
    ...
    refreshConfigSaveState();navigate('config'); renderConfig();
  };
  function saveCurrent(silent=false, validateCurrentStep=true) {
    const name = q('#cfgActivityName')?.value.trim(); if (name) { currentActivity.name = name; q('#configActivityName').textContent = name; }
    const detail = q('#cfgDetailEditor'); if (detail) config.detailHtml = detail.innerHTML.trim(); const notice=q('#cfgNoticeEditor');const noticeTitle=q('#cfgNoticeTitle');const noticeText=notice?String(notice.innerHTML||'').replace(/<[^>]*>/g,'').trim():String(config.noticeHtml||'').replace(/<[^>]*>/g,'').trim();const noticeTitleText=noticeTitle?noticeTitle.value.trim():String(config.noticeTitle||'').trim();if(validateCurrentStep&&stepIndex===1&&config.noticeEnabled&&Boolean(noticeTitleText)!==Boolean(noticeText)){if(!silent)showToast('默认全局提示请同时填写标题和内容，或全部留空');return false;}if(validateCurrentStep&&stepIndex===2&&config.projectModuleEnabled&&config.categoryEnabled&&config.categories.length===0){if(!silent)showToast('已开启项目分类，请至少新增 1 个项目分类后再继续');return false;}if(notice)config.noticeHtml=notice.innerHTML.trim();if(noticeTitle)config.noticeTitle=noticeTitleText;const noticeSeconds=q('#cfgNoticeSeconds');if(noticeSeconds)config.noticeSeconds=Math.max(0,Number(noticeSeconds.value)||0);
    ...
    saveActivityDraft();
    stepDirty=false;refreshConfigSaveState(); if(!silent) showToast('当前步骤已暂存，可查看游客端效果');return true;
  }
  ...
  function buildVisitorConfig() {
    ...
    const visitorSessions=(items,dateKey='default')=>sortSessions([...(items||[])]).map((item,index)=>{const [start,end]=item.time.split('-');const expired=dateKey<visitorToday;return {id:item.id||`${dateKey}-${index}`,time:start,endTime:end||'',name:item.name||'',state:expired?'closed':item.limit!==null&&item.booked>=item.limit?'full':'open',unlimited:item.limit===null,quota:item.limit===null?0:Math.max(0,item.limit-item.booked)};});
    ...
    const visibleDates=configuredDates;
    ...
    const previewConfig={
      ...
      dates:visibleDates.map(date=>{const items=config.sessionsByDate[date]||[];const unlimited=items.some(item=>item.limit===null);const full=!unlimited&&items.every(item=>item.limit!==null&&item.booked>=item.limit);const notice=effectiveNoticeFor(date);return {key:date,special:!!notice,notice,paused:config.pausedDates.includes(date),expired:false,full,unlimited,quota:items.reduce((sum,item)=>sum+(item.limit===null?0:Math.max(0,item.limit-item.booked)),0)};}),
  ...
  function openPreview() {
    if(stepDirty&&!saveCurrent(true,false))return;
  ...
  document.addEventListener('click', event => {
    ...
    const step = event.target.closest('[data-config-step]'); if(step){const target=Number(step.dataset.configStep);requestLeave(()=>{stepIndex=target;renderConfig();},false);return;}
    ...
    const pauseDate=event.target.closest('[data-pause-date]');if(pauseDate){const date=pauseDate.dataset.pauseDate;config.pausedDates=config.pausedDates.includes(date)?config.pausedDates.filter(item=>item!==date):[...config.pausedDates,date];closeLayer('configSheet');markDirty();renderConfig();showToast(config.pausedDates.includes(date)?'该日期已暂停新预约，切换页面后生效':'该日期已恢复预约，切换页面后生效');return;}
    const cancelDate=event.target.closest('[data-cancel-date]');if(cancelDate){const date=cancelDate.dataset.cancelDate;const count=config.dateBookedCounts[date];const activityName=currentActivity?.name||'当前活动';openDialog(`<h2>确认取消该日期全部预约？</h2><p>${date} 当前有 ${count} 笔有效预约。取消后不可恢复，并返还对应场次和项目名额。</p><label class="config-field"><span>取消原因 <b class="required">*</b></span><textarea id="cancelDateReason" placeholder="请输入取消原因"></textarea></label><section class="cancel-sms-preview"><b>确认后将向该日期 ${count} 条有效预约对应的游客分别发送短信</b><p id="cancelDateSmsPreview"></p></section><div class="dialog-actions"><button class="secondary" data-close="configDialog">暂不取消</button><button class="primary" data-confirm-cancel-date="${date}">确认全部取消</button></div>`);const preview=q('#cancelDateSmsPreview');const syncPreview=()=>{const reason=q('#cancelDateReason').value.trim();preview.textContent=`尊敬的游客，您好！${reason||'取消原因将显示在此处'}，${date}“${activityName}”暂停营业，给您带来不便敬请谅解，祝您生活愉快！`;preview.classList.toggle('placeholder',!reason);};q('#cancelDateReason').addEventListener('input',syncPreview);syncPreview();return;}
    const confirmCancelDate=event.target.closest('[data-confirm-cancel-date]');if(confirmCancelDate){const date=confirmCancelDate.dataset.confirmCancelDate;const reason=q('#cancelDateReason').value.trim();if(!reason){showToast('请填写取消原因');return;}config.dateBookedCounts[date]=0;config.sessionCancelNotifications=[...(config.sessionCancelNotifications||[]),{date,reason,status:'pending'}];closeLayer('configDialog');markDirty();renderConfig();showToast('该日期全部预约已取消，短信通知待发送');return;}
    ...
    const sessionDate=event.target.closest('[data-session-date]');if(sessionDate){const date=sessionDate.dataset.sessionDate;window.sessionDateSnapshot={date,sessions:config.sessionsByDate[date]?JSON.parse(JSON.stringify(config.sessionsByDate[date])):null,mode:config.sessionModes[date]?JSON.parse(JSON.stringify(config.sessionModes[date])):null,projectQuotas:JSON.parse(JSON.stringify(config.projectSessionQuotas)),projectBooked:JSON.parse(JSON.stringify(config.projectSessionBooked)),dirty:stepDirty,configSaved};openSessionDateEditor(date);return;}
    const cancelSessionDate=event.target.closest('[data-cancel-session-date]');if(cancelSessionDate){const snapshot=window.sessionDateSnapshot;if(snapshot){if(snapshot.sessions)config.sessionsByDate[snapshot.date]=snapshot.sessions;else delete config.sessionsByDate[snapshot.date];if(snapshot.mode)config.sessionModes[snapshot.date]=snapshot.mode;else delete config.sessionModes[snapshot.date];config.projectSessionQuotas=snapshot.projectQuotas;config.projectSessionBooked=snapshot.projectBooked;stepDirty=snapshot.dirty;configSaved=snapshot.configSaved;refreshConfigSaveState();}closeLayer('configSheet');renderConfig(true);showToast('已撤销本次当天场次调整');return;}
    const confirmSessionDate=event.target.closest('[data-confirm-session-date]');if(confirmSessionDate){const date=window.editingSessionDate;const sessions=config.sessionsByDate[date]||[];if(!sessions.length){delete config.sessionsByDate[date];delete config.sessionModes[date];showToast('当天至少需要 1 个场次');return;}config.sessionModes[date]={type:'configured',count:sessions.length,time:sessionTimeSummary(sessions)};config.previewDate=date;config.previewSessionId=sessions[0]?.id||'';window.sessionDateSnapshot=null;closeLayer('configSheet');markDirty();renderConfig(true);showToast('当天场次已确认，切换页面后自动暂存');return;}
    ...
    const saveQuickQuota=event.target.closest('[data-save-quick-quota]');if(saveQuickQuota){const item=config.sessionsByDate[window.editingSessionDate][window.adjustingSessionIndex];const setup=readProjectSetup('quick',item);const previousProjectIds=[...(item.projectIds||[])];const removedProjectIds=previousProjectIds.filter(projectId=>!setup.projectIds.includes(projectId));if(item.booked>0&&setup.separateProjectQuota!==Boolean(item.separateProjectQuota)){showToast('该场次已有预约，项目库存模式不能修改');return;}const protectedRemoval=removedProjectIds.find(projectId=>{const key=sessionProjectKey(window.editingSessionDate,item,projectId);const project=config.projects.find(candidate=>candidate.id===projectId);return (config.projectSessionBooked[key]||0)>0||!project?.enabled||!categoryIsEnabled(project.category);});if(protectedRemoval!==undefined){showToast('已预约项目或已禁用项目不能取消关联');return;}if(setup.separateProjectQuota){let invalid='';let total=0;setup.projectIds.forEach(projectId=>{const key=sessionProjectKey(window.editingSessionDate,item,projectId);const booked=config.projectSessionBooked[key]||0;const quota=setup.quotas[String(projectId)];if(!Number.isFinite(quota)||quota<1)invalid='请填写每个已选项目的有效名额';else if(quota<booked)invalid=`${config.projects.find(p=>p.id===projectId)?.name||'项目'}名额不能低于已预约 ${booked} 人`;else total+=quota;});if(invalid){showToast(invalid);return;}item.projectIds=setup.projectIds;item.separateProjectQuota=true;item.limit=total;removedProjectIds.forEach(projectId=>{delete config.projectSessionQuotas[sessionProjectKey(window.editingSessionDate,item,projectId)];delete config.projectSessionBooked[sessionProjectKey(window.editingSessionDate,item,projectId)];});Object.entries(setup.quotas).forEach(([projectId,quota])=>config.projectSessionQuotas[sessionProjectKey(window.editingSessionDate,item,projectId)]=quota);}else{const mode=document.querySelector('input[name="quickQuotaMode"]:checked')?.value||'unlimited';const value=q('#quickSessionQuota').value;const next=mode==='unlimited'?null:Number(value);if(mode==='limited'&&(!value||next<Math.max(1,item.booked))){showToast(item.booked?`名额不能低于已预约 ${item.booked} 人`:'请输入有效名额');return;}item.projectIds=setup.projectIds;item.separateProjectQuota=false;item.limit=next;removedProjectIds.forEach(projectId=>{delete config.projectSessionQuotas[sessionProjectKey(window.editingSessionDate,item,projectId)];delete config.projectSessionBooked[sessionProjectKey(window.editingSessionDate,item,projectId)];});}markDirty();renderConfig(true);openSessionDateEditor(window.editingSessionDate);return;}
    ...
    const confirmDeleteProject=event.target.closest('[data-confirm-delete-project]');if(confirmDeleteProject){config.projects.splice(Number(confirmDeleteProject.dataset.confirmDeleteProject),1);closeLayer('configDialog');markDirty();renderConfig(true);showToast('项目已删除，切换页面后自动暂存');return;}
    ...
    if(type==='replaceLogo'){
      ...
      input.addEventListener('change',()=>{const file=input.files?.[0];if(!file)return;const reader=new FileReader();reader.onload=()=>{config.draftLogo=reader.result;markDirty();renderConfig();showToast('活动LOGO已更新，切换页面后同步到列表');};reader.readAsDataURL(file);});
      input.click();return;
    }
    if(type==='removeLogo'){config.draftLogo=null;markDirty();renderConfig();showToast('已删除LOGO，列表将显示活动名首字');return;}
    if(type==='replaceCover'){
      const input=document.createElement('input');input.type='file';input.accept='image/*';
      input.addEventListener('change',()=>{const file=input.files?.[0];if(!file)return;const reader=new FileReader();reader.onload=()=>{config.draftCover=reader.result;markDirty();renderConfig();showToast('封面图片已更新');};reader.readAsDataURL(file);});input.click();return;
    }
    if(type==='removeCover'){config.draftCover=null;markDirty();renderConfig();showToast('封面图片已删除');return;}
    ...
    if(type==='addSession'){config.sessions.push({name:'',time:'14:30',limit:null,booked:0});markDirty();renderConfig();showToast('已添加场次，切换页面后更新预览');return;}
  ...
  q('#configPrev').addEventListener('click',()=>{requestLeave(()=>{if(stepIndex===0){navigate(window.configReturnPage||'activities');return;}stepIndex--;renderConfig();},false);});
  q('#configNext').addEventListener('click',()=>{requestLeave(()=>{if(stepIndex<steps.length-1){stepIndex++;renderConfig();}},true);});
  q('#configContent').addEventListener('input',()=>markDirty());
  q('#configContent').addEventListener('change',()=>markDirty());
  document.addEventListener('click',event=>{if(event.target.id==='savePublishConfig'){if(stepDirty&&!saveCurrent(true,false))return;const failed=publishChecks().filter(item=>!item[1]);if(failed.length){showToast(`还有 ${failed.length} 项发布检查未通过`);q('.publish-check .missing')?.scrollIntoView({behavior:'smooth',block:'center'});return;}const wasPublished=currentActivity.status==='published';const visitorConfig=buildVisitorConfig();localStorage.setItem('scenicPublishedConfig',JSON.stringify(visitorConfig));const publishedMap=JSON.parse(localStorage.getItem('scenicPublishedActivitiesV34')||'{}');publishedMap[String(currentActivity.id)]={activityId:String(currentActivity.id),publishedAt:new Date().toISOString(),config:visitorConfig};localStorage.setItem('scenicPublishedActivitiesV34',JSON.stringify(publishedMap));currentActivity.status='published';isNewActivity=false;configSaved=true;saveActivityDraft();window.syncVisitorActivityCatalog?.(currentActivity.id);navigate('activities');renderActivities();showToast(wasPublished?'保存并发布成功，线上版本已更新':'保存并发布成功，活动已上架');}});
  document.addEventListener('click',event=>{if(event.target.id==='saveConfig'){if(stepDirty&&!saveCurrent(true,false))return;configSaved=true;saveActivityDraft();navigate('activities');renderActivities();showToast('配置已保存，活动保持下架');return;}if(event.target.id==='offlineConfig'){openDialog('<h2>确认下架活动？</h2><p>下架后停止接受新预约；历史预约仍可查看、修改和取消。当前暂存配置会保留，之后可继续编辑或重新发布。</p><div class="dialog-actions"><button class="secondary" data-close="configDialog">暂不下架</button><button class="danger" id="confirmOfflineConfig">确认下架</button></div>');return;}if(event.target.id==='confirmOfflineConfig'){if(stepDirty&&!saveCurrent(true,false))return;currentActivity.status='offline';saveActivityDraft();window.syncVisitorActivityCatalog?.(null,currentActivity.id);closeLayer('configDialog');navigate('activities');renderActivities();showToast('活动已下架，停止接受新预约');return;}if(event.target.id==='deleteConfig'){if(Number(currentActivity?.totalPeople||0)>0){showToast('该活动已有预约数据，只能保持下架，不允许删除');return;}openDialog(`<h2>确认删除活动？</h2><p>“${currentActivity?.name||'当前活动'}”尚无预约数据。删除后活动及全部配置不可恢复。</p><div class="dialog-actions"><button class="secondary" data-close="configDialog">取消</button><button class="danger" id="confirmDeleteConfig">确认删除</button></div>`);return;}if(event.target.id==='confirmDeleteConfig'){if(Number(currentActivity?.totalPeople||0)>0){closeLayer('configDialog');showToast('当前活动不满足删除条件');return;}const deletedId=currentActivity?.id;const index=activities.findIndex(item=>item.id===deletedId);if(index>=0)activities.splice(index,1);try{const publishedMap=JSON.parse(localStorage.getItem('scenicPublishedActivitiesV34')||'{}');delete publishedMap[String(deletedId)];localStorage.setItem('scenicPublishedActivitiesV34',JSON.stringify(publishedMap));const drafts=readActivityDrafts();delete drafts[String(deletedId)];localStorage.setItem(draftStorageKey,JSON.stringify(drafts));}catch(error){}window.syncVisitorActivityCatalog?.(null,deletedId);closeLayer('configDialog');currentActivity=null;navigate('activities');renderActivities();showToast('活动已删除');}});
  ...
  function saveActivityDraft(){if(!currentActivity?.id)return;const drafts=readActivityDrafts();drafts[currentActivity.id]={config,currentActivity:{...currentActivity},configSaved};localStorage.setItem(draftStorageKey,JSON.stringify(drafts));localStorage.removeItem('scenicDraftConfigV34');}
  ...
  try{
    ...
    if(saved?.config){Object.assign(config,saved.config);configSaved=saved.configSaved ?? true;Object.entries(config.sessionsByDate||{}).forEach(([date,sessions])=>ensureSessionIds(date,sessions));}
    if(saved?.currentActivity&&saved.currentActivity.id===currentActivity.id)restoreActivityDraft(currentActivity,saved.currentActivity);
