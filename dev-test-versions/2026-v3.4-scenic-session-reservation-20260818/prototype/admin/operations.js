(() => {
  ...
  const activityNameFor = item => (activities.find(activity => activity.id === item?.activityId) || currentActivity || activities[0]).name;
  const cancelSmsText = (item, reason) => `尊敬的游客，您好！${reason}，${item.date}“${activityNameFor(item)}”暂停营业，给您带来不便敬请谅解，祝您生活愉快！`;
  function renderCancelSmsPreview(){
    const preview=q('#adminCancelSmsPreview'); if(!preview||!currentBooking)return;
    const reason=q('#adminCancelReason').value.trim();
    const renderedReason=reason ? escapeHtml(reason) : '<span class="placeholder">取消原因将显示在此处</span>';
    preview.innerHTML=`尊敬的游客，您好！${renderedReason}，${escapeHtml(currentBooking.date)}“${escapeHtml(activityNameFor(currentBooking))}”暂停营业，给您带来不便敬请谅解，祝您生活愉快！`;
  }
  function prepareAdminCancel(){ q('#adminCancelReason').value=''; renderCancelSmsPreview(); openLayer('cancelBooking'); }
  ...
  function renderAdminBooking() {
    ...
    q('#adminBookingInfo').innerHTML = detailRow('提交时间',item.created)+detailRow('最近修改时间',item.modified||'-')+(cancelled?detailRow('取消时间',item.cancelTime||'未记录')+detailRow('取消方式',item.cancelType==='visitor'?'游客取消':'管理员取消')+detailRow('取消原因',item.cancelReason||'未填写')+(item.cancelType==='admin'?detailRow('短信通知',item.smsNotice?.status==='sent'?'已发送':item.smsNotice?.status==='failed'?'发送失败':'待发送'):''):'')+detailRow('预约游玩日期',item.date)+detailRow('场次名称',item.sessionName||'未设置场次名称')+detailRow('场次时间',item.session)+(item.category?detailRow('分类',item.category):'')+(item.project?detailRow('项目',item.project):'')+detailRow('实际参与人数',`${item.people} 人`);
  ...
  document.addEventListener('click', event => {
    ...
    const rosterCancel=event.target.closest('[data-roster-cancel]'); if(rosterCancel){ currentBooking=bookings.find(item=>item.id===Number(rosterCancel.dataset.rosterCancel)); prepareAdminCancel(); return; }
  ...
  q('#adminCancelBooking').addEventListener('click',prepareAdminCancel);
  q('#adminCancelReason').addEventListener('input',renderCancelSmsPreview);
  q('#confirmCancelBooking').addEventListener('click',()=>{const reason=q('#adminCancelReason').value.trim();if(!reason){showToast('请填写取消原因');return;}currentBooking.status='cancelled';window.refreshTodayPeople?.();currentBooking.cancelType='admin';currentBooking.cancelActor=currentAdminName;currentBooking.cancelTime='2026-08-20 13:16';currentBooking.cancelReason=reason;currentBooking.smsNotice={status:'pending',phone:currentBooking.phone,text:cancelSmsText(currentBooking,reason),createdAt:currentBooking.cancelTime};currentBooking.operationLogs=currentBooking.operationLogs||[{type:'create',time:currentBooking.created,actor:'游客'}];currentBooking.operationLogs.push({type:'admin_cancel',time:currentBooking.cancelTime,actor:currentAdminName});if(rosterSession&&currentBooking.date===rosterSession.date&&currentBooking.session===rosterSession.time){rosterSession.booked=Math.max(0,rosterSession.booked-currentBooking.people);rosterSession.orders=Math.max(0,rosterSession.orders-1);}closeLayer('cancelBooking');if(currentPage==='roster')renderRoster();else renderAdminBooking();showToast('预约已取消，短信通知待发送');});
