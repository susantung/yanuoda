(() => {
  ...
  const syncExportType=()=>{const type=q('input[name="exportType"]:checked')?.value||'records';q('#exportStatusField').hidden=type==='roster';};
  ...
  const rosterExportSheets=(start,end)=>{const sessions=sessionSamples.filter(item=>item.date>=start&&item.date<=end).sort((a,b)=>a.date.localeCompare(b.date)||a.time.localeCompare(b.time));const active=bookings.filter(item=>item.activityId===(currentActivity||activities[0]).id&&item.status==='active'&&item.date>=start&&item.date<=end);const summary=[['预约日期','星期','场次名称','开始时间','结束时间','状态','总名额','已预约人数','剩余名额','预约单数','姓名摘要'],...sessions.map(session=>{const related=active.filter(item=>item.date===session.date&&item.session===session.time);const weekday=['周日','周一','周二','周三','周四','周五','周六'][new Date(`${session.date}T00:00:00`).getDay()];const [begin='',finish='']=session.time.split('-');return [session.date,weekday,session.name||'',begin,finish,session.state==='expired'?'已过期':session.state==='paused'?'已暂停':'开放中',session.limit===null?'不限名额':session.limit,session.booked,session.limit===null?'不限名额':Math.max(0,session.limit-session.booked),session.orders,related.map(item=>item.name).filter(Boolean).join('、')];})];const detailHeaders=['游客预约号','预约日期','场次名称','开始时间','结束时间','分类','项目','实际参与人数','预约人姓名','手机号','身份证号','提交时间'];const detail=[detailHeaders,...active.map(item=>{const [begin='',finish='']=item.session.split('-');return [item.number,item.date,item.sessionName||'',begin,finish,item.category||'',item.project||'',item.people,snapshotValue(item,'name'),snapshotValue(item,'phone'),snapshotValue(item,'idNumber'),item.created];})];return {'场次汇总':summary,'预约明细':detail};};
