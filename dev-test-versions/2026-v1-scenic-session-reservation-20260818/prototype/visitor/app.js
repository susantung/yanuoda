...
function applyExternalPreview(payload) {
  ...
  const firstBookableDate = dateData.find(item => !item.paused && !item.expired && !item.full) || dateData[0];
  state.date = firstBookableDate?.key || todayDateKey; state.dateMonth = firstBookableDate?.month || 8; state.session = null;
...
try{publishedPreviewConfig=JSON.parse(localStorage.getItem('scenicPublishedConfig')||'null');}catch(error){publishedPreviewConfig=null;}
