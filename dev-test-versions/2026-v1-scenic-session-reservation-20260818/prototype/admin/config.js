(() => {
  ...
  const config = {
    noticeEnabled:true, noticeScope:'special', noticeTitle:'溪降预约必读须知', noticeSeconds:3, dateMode:'custom', projectModuleEnabled:true, categoryEnabled:true, visitorCancel:true, bookingCutoffMode:'start', bookingCutoffValue:30, bookingCutoffUnit:'minute',
    ...
    projectTheme:'选择体验线路', projectInventoryRelation:'shared', projectSessionQuotas:{}, projectSessionBooked:{}, projectSessionEnabled:{},
    defaultProject:{id:'system-default',name:'系统默认项目',system:true},
    detailHtml:'<p>尊敬的游客，该门票包含“溪降”体验项目。为确保安全与体验质量，请提前选择预约时段，并仔细阅读预约说明。</p><img src="../visitor/assets/activity-source.jpg" alt="溪降项目环境示例图">', noticeHtml:'<p>当日部分区域有安全提示，请根据同行人员情况谨慎选择是否预约。</p><p>预约成功后请提前15分钟到达溪降接待处。</p>', draftLogo:null, draftCover:null,
    sessions:[
      {name:'溪降 1 场',time:'09:30-10:30',limit:15,booked:12},
      {name:'溪降 2 场',time:'10:30-11:30',limit:15,booked:15},
      {name:'溪降 3 场',time:'11:30-12:30',limit:15,booked:0}
    ],
    projects:[
  ...
  const cloneDateSessions = (sourceDate,targetDate) => {
    ...
    const cloned=source.map((item,index)=>({...item,id:`${targetDate}-copy-${stamp}-${index}`,booked:0}));
    cloned.forEach((target,index)=>{
      if(!target.separateProjectQuota)return;
      config.projects.filter(project=>project.enabled).forEach(project=>{
  ...
  const sessionProjectItems = () => {
    if(!config.projectModuleEnabled)return [config.defaultProject];
    const projects=config.projects.filter(item=>item.enabled);
    return projects.length?projects:[config.defaultProject];
  };
  const sessionProjectRows = (date,sessionIndex,session) => {
    const projects=sessionProjectItems();ensureSessionIds(date,config.sessionsByDate[date]||[]);
    const defaultOnly=projects.length===1&&projects[0].system;
    if(defaultOnly)return `<div class="session-project-panel default-session-project"><div class="session-project-panel-head"><b>场次项目</b><span>系统默认项目</span></div><div class="session-project-row"><span class="session-project-initial">默</span><div><b>系统默认项目</b><small>名额与当前场次一致 · ${session.limit===null?'不限额':`${session.limit}名`}</small></div><em>已启用</em></div></div>`;
    const rows=projects.map(project=>{const key=sessionProjectKey(date,session,project.id);const enabled=config.projectSessionEnabled[key]!==false;const booked=config.projectSessionBooked[key]||0;const quota=config.projectSessionQuotas[key];return `<div class="session-project-row ${enabled?'':'is-disabled'}"><span class="session-project-initial">${displayInitial(project.name)}</span><div><b title="${project.name}">${project.name}</b><small>${booked?`已预约 ${booked} 人`:'暂无预约'}</small></div><label class="session-project-quota"><input data-session-project-quota data-date="${date}" data-session="${session.id}" data-project="${project.id}" type="number" min="${Math.max(1,booked)}" inputmode="numeric" value="${quota??''}" placeholder="不限额" ${enabled?'':'disabled'}><span>名额</span></label><button class="mini-switch session-project-switch ${enabled?'on':''}" data-session-project-toggle data-date="${date}" data-session="${session.id}" data-project="${project.id}" aria-label="${project.name}${enabled?'已启用':'已禁用'}"></button></div>`;}).join('');
    return `<div class="session-project-panel"><div class="session-project-panel-head"><b>场次项目与名额</b><span>${projects.length}个总体启用项目</span></div>${rows}</div>`;
  };
  ...
  const publishChecks = () => {
    ...
    const projectSessionReady=projectSessions.every(()=>overallProjects.length>0);
    const quotaSafe=projectSessions.every(({date,session})=>!session.separateProjectQuota||overallProjects.every(project=>{const key=sessionProjectKey(date,session,project.id);const quota=config.projectSessionQuotas[key],booked=config.projectSessionBooked[key]||0;return quota===undefined||quota>=booked;}));
    return [
      ...
      ['项目配置',!config.projectModuleEnabled||overallProjects.length>0,config.projectModuleEnabled?`总体启用 ${overallProjects.length} 个项目`:'使用系统默认项目'],
      ['场次项目',!config.projectModuleEnabled||(projectSessionReady&&quotaSafe),!config.projectModuleEnabled?'使用系统默认项目':projectSessionReady&&quotaSafe?'项目可约，单独项目名额有效':'请检查单独项目名额'],
  ...
  const views = [
    ...
    () => `${heading(2,'先配置项目资料和分类，再进入日期与场次设置。')}
      <section class="config-card project-overview-card"><h3>项目模块</h3>${switchRow('projectModuleEnabled','启用项目模块','开启后游客需选择一个展示中的项目；关闭后游客端不展示项目模块')}${config.projectModuleEnabled?`<label class="config-field project-theme-field"><span>模块名称 <small><b class="required">*</b> 游客端标题</small></span><input id="projectThemeInput" type="text" value="${config.projectTheme}"></label><p class="selection-rule-tip"><i>i</i>游客每次预约固定选择 1 个项目</p>`:`<div class="default-project-card"><span class="default-project-icon">默</span><div><b>关闭后的预约与库存</b><span>游客端不展示项目选择；系统默认项目承接预约，库存直接读取对应日期和场次的名额。</span></div></div>`}</section>
    ...
    () => `${heading(5,'配置游客填写项、人员统计方式、预约开放截止和取消规则。')}
      ...
      <section class="config-card"><h3>预约开放与截止</h3><div class="fixed-setting-row"><span>开放规则</span><b>立即开放</b></div><label class="config-field"><span>预约截止</span><select id="bookingCutoffMode"><option value="start" ${config.bookingCutoffMode==='start'?'selected':''}>到场次开始时间截止</option><option value="advance" ${config.bookingCutoffMode==='advance'?'selected':''}>场次开始前 N 分钟/小时截止</option></select></label><div id="bookingCutoffAdvance" class="two-fields" ${config.bookingCutoffMode==='advance'?'':'hidden'}><label class="config-field"><span>提前数值</span><input id="bookingCutoffValue" type="number" min="1" value="${config.bookingCutoffValue}"></label><label class="config-field"><span>单位</span><select id="bookingCutoffUnit"><option value="minute" ${config.bookingCutoffUnit==='minute'?'selected':''}>分钟</option><option value="hour" ${config.bookingCutoffUnit==='hour'?'selected':''}>小时</option></select></label></div><p class="config-hint">游客与管理员均可修改预约资料；日期、场次、分类和项目不可修改，选错需取消后重新预约。</p></section>
      <section class="config-card"><h3>游客取消规则</h3>${switchRow('visitorCancel','允许游客取消预约','管理员始终可以取消预约',true)}<label class="config-field" style="margin-top:10px"><span>游客取消截止</span><select><option>场次开始前30分钟</option><option>到场次开始时间</option></select></label><p class="config-hint">修改实际参与人数时重新校验库存；取消成功后返还场次及项目名额。</p></section>`,

    () => `${heading(6,'发布前集中检查活动资料、排期、项目、游客资料和预约规则。')}
      <section class="config-card"><div class="publish-page-head"><div><h3>发布检查</h3><p>全部必备项通过后才允许发布或发布更新</p></div><span>${publishChecks().filter(item=>item[1]).length}/${publishChecks().length}</span></div><div class="publish-check detailed">${publishChecks().map(([title,passed,note])=>`<div class="${passed?'':'missing'}"><i>${passed?'✓':'!'}</i><span><b>${title}</b><small>${note}</small></span></div>`).join('')}</div></section><section class="config-card"><h3>发布结果</h3><p class="publish-summary">发布后接受新预约；活动下架后停止新预约，历史预约仍可查看、修改和取消，管理员可编辑后重新发布。</p><button class="publish-button" id="publishConfig" ${publishChecks().some(item=>!item[1])?'disabled':''}>${currentActivity?.status==='published'?'发布更新':'发布预约活动'}</button></section>`
  ];
  ...
  function renderConfig(preserveScroll=false) {
    ...
    if(stepIndex===3){const title=q('.session-config-title');if(title&&!q('.session-config-head-actions',title)){const actions=document.createElement('div');actions.className='session-config-head-actions';actions.innerHTML=`<button data-manage-session-templates>模板管理 ${config.sessionTemplates.length}</button>`;title.appendChild(actions);}}
    q('#configPrev').textContent = stepIndex === 0 ? '返回列表' : '上一步';
    q('#configSave').textContent = '保存';q('#configNext').hidden=stepIndex===steps.length-1;
  ...
  function openProjectEditor(index=null){
    const item=index===null?{name:'',category:'未分类',image:false,description:'',enabled:true,booked:0}:config.projects[index];
    ...
    openSheet(`<h2>${index===null?'添加项目':'编辑项目'}</h2>
      ...
      <label class="config-field"><span>所属分类 <small>选填</small></span><select id="sheetProjectCategory"><option value="未分类">未分类</option>${config.categories.map(name=>`<option ${name===item.category?'selected':''}>${name}</option>`).join('')}</select></label>
  ...
  function openProjectSessionInventory(date=null,sessionIndex=0){
    ...
    openSheet(`<h2>按日期和场次配置项目名额</h2>
      ...
      <div class="project-stock-list">${projects.map(item=>{const key=sessionProjectKey(selectedDate,session,item.id);return `<label><span><b>${item.name}</b><small>${item.category||'未分类'}</small></span><input data-project-stock-id="${item.id}" type="number" min="1" inputmode="numeric" value="${config.projectSessionQuotas[key]??''}" placeholder="不限额"></label>`;}).join('')||'<p class="config-hint">请先启用至少一个项目。</p>'}</div>
  ...
  function renderPreview() {
    ...
    if(previewTab==='form') {
      ...
      container.innerHTML=`<div class="preview-source-note">以下字段来自“C06 游客预约配置”的当前配置，仅展示页面效果，不提交预约</div><section class="visitor-section"><div class="visitor-section-title"><h3>本次预约</h3><span>日期和场次不可修改</span></div><div class="visitor-summary"><div><span>预约日期</span><b>2026-08-15</b></div><div><span>预约场次</span><b>${config.sessions[0]?.time||'09:30-10:30'}</b></div>${firstProject?`<div><span>分类</span><b>${firstProject.category||'未分类'}</b></div><div><span>项目</span><b>${firstProject.name}</b></div>`:''}</div></section><section class="visitor-section"><div class="visitor-section-title"><h3>预约人信息</h3><span>${config.fields.some(f=>f.type==='多人/团体')?'多人统计模式':'单人统计模式'}</span></div>${config.fields.map(previewField).join('')}</section><div class="preview-bottom"><button disabled>仅查看展示效果</button></div>`; return;
    }
    const visibleProjects=enabledProjects();
  ...
  function buildVisitorConfig() {
    ...
    const projectsBySession=Object.fromEntries(visibleDates.map(date=>{
      ...
      return [date,Object.fromEntries(items.map(session=>[
        session.id,
        Object.fromEntries(visibleProjects.map(project=>{
  ...
  function openPreview() {
    ...
    const previewConfig={...buildVisitorConfig(),previewOnly:true,previewPage:stepIndex===5?'form':'select'};
    ...
    frame.src=`../visitor/index.html?configPreview=${Date.now()}`;
  }
  ...
  document.addEventListener('click', event => {
    ...
    const sessionDate=event.target.closest('[data-session-date]');if(sessionDate){const date=sessionDate.dataset.sessionDate;window.sessionDateSnapshot={date,sessions:config.sessionsByDate[date]?JSON.parse(JSON.stringify(config.sessionsByDate[date])):null,mode:config.sessionModes[date]?JSON.parse(JSON.stringify(config.sessionModes[date])):null,dirty:stepDirty};openSessionDateEditor(date);return;}
    const cancelSessionDate=event.target.closest('[data-cancel-session-date]');if(cancelSessionDate){const snapshot=window.sessionDateSnapshot;if(snapshot){if(snapshot.sessions)config.sessionsByDate[snapshot.date]=snapshot.sessions;else delete config.sessionsByDate[snapshot.date];if(snapshot.mode)config.sessionModes[snapshot.date]=snapshot.mode;else delete config.sessionModes[snapshot.date];stepDirty=snapshot.dirty;q('#configSaveState').textContent=stepDirty?'当前步骤未保存':'配置已保存';}closeLayer('configSheet');renderConfig(true);showToast('已撤销本次当天场次调整');return;}
    const confirmSessionDate=event.target.closest('[data-confirm-session-date]');if(confirmSessionDate){const date=window.editingSessionDate;const sessions=config.sessionsByDate[date]||[];if(!sessions.length){delete config.sessionsByDate[date];delete config.sessionModes[date];showToast('当天至少需要 1 个场次');return;}config.sessionModes[date]={type:'configured',count:sessions.length,time:sessionTimeSummary(sessions)};window.sessionDateSnapshot=null;closeLayer('configSheet');markDirty();renderConfig(true);showToast('当天场次已确认，请保存当前步骤');return;}
    ...
    const addSessionDate=event.target.closest('[data-add-session-date]');if(addSessionDate){window.editingSessionDate=addSessionDate.dataset.addSessionDate;openSheet(`<h2>添加单个场次</h2><label class="config-field"><span>场次名称 <small>选填，最多10个字</small></span><input id="sheetSessionName" type="text" maxlength="10" placeholder="请输入场次名称"></label><div class="two-fields"><label class="config-field"><span>开始时间 <b class="required">*</b></span><input id="sheetSessionStart" type="time" value=""></label><label class="config-field"><span>结束时间 <small>选填</small></span><input id="sheetSessionEnd" type="time" value=""></label></div><label class="config-field"><span>场次名额 <small>选填，留空为不限</small></span><input id="sheetSessionLimit" type="number" min="1" value="" placeholder="请输入名额"></label><div class="config-sheet-actions"><button class="secondary" data-back-date-editor>取消</button><button class="primary" data-create-session>添加场次</button></div>`);return;}
    const createSession=event.target.closest('[data-create-session]');if(createSession){const date=window.editingSessionDate;const start=q('#sheetSessionStart').value.trim();const end=q('#sheetSessionEnd').value.trim();if(!start){showToast('请填写开始时间');return;}if(end&&end<=start){showToast('结束时间必须晚于开始时间');return;}config.sessionsByDate[date]=config.sessionsByDate[date]||[];config.sessionsByDate[date].push({id:`${date}-session-${Date.now()}`,name:q('#sheetSessionName').value.trim(),time:end?`${start}-${end}`:start,limit:q('#sheetSessionLimit').value===''?null:Number(q('#sheetSessionLimit').value),booked:0,separateProjectQuota:false});const list=config.sessionsByDate[date];sortSessions(list);config.sessionModes[date]={type:'configured',count:list.length,time:sessionTimeSummary(list)};markDirty();renderConfig(true);openSessionDateEditor(date);return;}
    const batchAddSession=event.target.closest('[data-batch-add-session-date]');if(batchAddSession){window.editingSessionDate=batchAddSession.dataset.batchAddSessionDate;openSheet(`<h2>批量添加场次</h2><div class="two-fields"><label class="config-field"><span>开始时间 <b class="required">*</b></span><input id="batchSessionStart" type="time" value=""></label><label class="config-field"><span>结束时间 <b class="required">*</b></span><input id="batchSessionEnd" type="time" value=""></label></div><label class="config-field"><span>场次间隔</span><select id="batchSessionInterval">${Array.from({length:36},(_,index)=>(index+1)*5).map(minutes=>`<option value="${minutes}" ${minutes===30?'selected':''}>${minutes}分钟</option>`).join('')}</select></label><div class="quota-mode-card"><span>统一场次名额</span><div class="quota-mode-options"><label><input type="radio" name="batchQuotaMode" value="unlimited" checked> 不限额</label><label><input type="radio" name="batchQuotaMode" value="limited"> 统一限额</label></div><input id="batchSessionQuota" class="quota-number-input" type="number" min="1" inputmode="numeric" placeholder="请输入统一名额" disabled></div><div class="batch-session-result" id="batchSessionResult">选择开始和结束时间后计算拆分数量</div><div class="config-sheet-actions"><button class="secondary" data-back-date-editor>取消</button><button class="primary" data-confirm-batch-sessions>确认添加</button></div>`);return;}
    const confirmBatchSessions=event.target.closest('[data-confirm-batch-sessions]');if(confirmBatchSessions){const start=q('#batchSessionStart').value,end=q('#batchSessionEnd').value,interval=Number(q('#batchSessionInterval').value);const quotaMode=document.querySelector('input[name="batchQuotaMode"]:checked')?.value||'unlimited';const quotaValue=q('#batchSessionQuota').value;const limit=quotaMode==='unlimited'?null:Number(quotaValue);const toMinutes=value=>{const [h,m]=value.split(':').map(Number);return h*60+m;};if(!start||!end){showToast('请选择开始和结束时间');return;}if(quotaMode==='limited'&&(!quotaValue||limit<1)){showToast('请输入统一场次名额');return;}const total=toMinutes(end)-toMinutes(start);if(total<=0){showToast('结束时间必须晚于开始时间');return;}if(total%interval!==0){showToast(`总时长${total}分钟不能按${interval}分钟整除，请调整结束时间或间隔`);return;}const count=total/interval;const format=value=>`${String(Math.floor(value/60)).padStart(2,'0')}:${String(value%60).padStart(2,'0')}`;const base=toMinutes(start);const date=window.editingSessionDate;const batchId=Date.now();const generated=Array.from({length:count},(_,index)=>({id:`${date}-batch-${batchId}-${index}`,name:'',time:`${format(base+index*interval)}-${format(base+(index+1)*interval)}`,limit,booked:0,separateProjectQuota:false}));config.sessionsByDate[date]=config.sessionsByDate[date]||[];config.sessionsByDate[date].push(...generated);const list=config.sessionsByDate[date];sortSessions(list);config.sessionModes[date]={type:'configured',count:list.length,time:sessionTimeSummary(list)};markDirty();renderConfig(true);openSessionDateEditor(date);return;}
    const quickQuota=event.target.closest('[data-quick-quota]');if(quickQuota){window.adjustingSessionIndex=Number(quickQuota.dataset.quickQuota);const item=config.sessionsByDate[window.editingSessionDate][window.adjustingSessionIndex];const projectItems=sessionProjectItems().filter(project=>!project.system);const projectRows=projectItems.map(project=>{const key=sessionProjectKey(window.editingSessionDate,item,project.id);const booked=config.projectSessionBooked[key]||0;const quota=config.projectSessionQuotas[key];return `<label class="quick-project-quota-row"><span><b>${project.name}</b><small>${booked?`已预约 ${booked} 人`:'暂无预约'}</small></span><input data-quick-project-quota="${project.id}" type="number" min="${Math.max(1,booked)}" inputmode="numeric" value="${quota??''}" placeholder="不限额"></label>`;}).join('');openSheet(`<h2>快捷设置场次名额</h2><p class="config-hint quick-quota-time">${item.time}${item.booked?` · 已预约 ${item.booked} 人`:''}</p><div class="quota-mode-card"><div class="quota-mode-options"><label><input type="radio" name="quickQuotaMode" value="unlimited" ${item.limit===null?'checked':''}> 不限额</label><label><input type="radio" name="quickQuotaMode" value="limited" ${item.limit!==null?'checked':''}> 限额</label></div><input id="quickSessionQuota" class="quota-number-input" type="number" inputmode="numeric" min="${Math.max(1,item.booked)}" value="${item.limit??''}" placeholder="请输入名额" ${item.limit===null?'disabled':''}></div>${projectItems.length?`<label class="separate-project-quota"><span><b>单独设置项目名额</b><small>关闭时，所有项目共用本场次名额</small></span><input id="separateProjectQuota" type="checkbox" ${item.separateProjectQuota?'checked':''}></label><div id="quickProjectQuotaPanel" class="quick-project-quota-panel" ${item.separateProjectQuota?'':'hidden'}>${projectRows}</div>`:''}<div class="config-sheet-actions"><button class="secondary" data-back-date-editor>取消</button><button class="primary" data-save-quick-quota>保存名额</button></div>`);return;}
    const saveQuickQuota=event.target.closest('[data-save-quick-quota]');if(saveQuickQuota){const item=config.sessionsByDate[window.editingSessionDate][window.adjustingSessionIndex];const mode=document.querySelector('input[name="quickQuotaMode"]:checked')?.value||'unlimited';const value=q('#quickSessionQuota').value;const next=mode==='unlimited'?null:Number(value);if(mode==='limited'&&(!value||next<Math.max(1,item.booked))){showToast(item.booked?`名额不能低于已预约 ${item.booked} 人`:'请输入有效名额');return;}item.limit=next;item.separateProjectQuota=Boolean(q('#separateProjectQuota')?.checked);if(item.separateProjectQuota){let invalid='';qa('[data-quick-project-quota]',q('#configSheet')).forEach(input=>{const key=sessionProjectKey(window.editingSessionDate,item,input.dataset.quickProjectQuota);const booked=config.projectSessionBooked[key]||0;if(input.value!==''&&Number(input.value)<Math.max(1,booked))invalid=`项目名额不能低于已预约 ${booked} 人`;});if(invalid){showToast(invalid);return;}qa('[data-quick-project-quota]',q('#configSheet')).forEach(input=>{const key=sessionProjectKey(window.editingSessionDate,item,input.dataset.quickProjectQuota);if(input.value==='')delete config.projectSessionQuotas[key];else config.projectSessionQuotas[key]=Number(input.value);});}else{sessionProjectItems().filter(project=>!project.system).forEach(project=>{delete config.projectSessionQuotas[sessionProjectKey(window.editingSessionDate,item,project.id)];});}markDirty();renderConfig(true);openSessionDateEditor(window.editingSessionDate);return;}
    ...
    const confirmSaveDateTemplate=event.target.closest('[data-confirm-save-date-template]');if(confirmSaveDateTemplate){const name=q('#sessionTemplateName').value.trim();if(!name){showToast('请输入模板名称');return;}const sessions=config.sessionsByDate[window.templateSourceDate]||[];if(!sessions.length){showToast('当天还没有可保存的场次');return;}config.sessionTemplates.push({name,sessions:sessions.map(item=>({...item,booked:0}))});closeLayer('configDialog');markDirty();showToast('场次模板已保存');return;}
    ...
    const saveProject=event.target.closest('[data-save-project]');if(saveProject){const name=q('#sheetProjectName').value.trim();if(!name){showToast('请输入项目名称');return;}const original=saveProject.dataset.saveProject==='new'?null:config.projects[Number(saveProject.dataset.saveProject)];const next={id:original?.id||Date.now(),name:name.slice(0,30),category:q('#sheetProjectCategory').value||'未分类',image:window.projectDraftImage||false,description:q('#sheetProjectDescription').value.trim().slice(0,60),enabled:original?.enabled??true,booked:original?.booked||0};if(original)Object.assign(original,next);else config.projects.push(next);closeLayer('configSheet');markDirty();renderConfig(true);showToast(original?'项目已更新，请保存当前步骤':'项目已添加并默认启用，请保存当前步骤');return;}
  ...
  document.addEventListener('change',event=>{
    ...
    if(event.target.name==='projectInventoryRelation'){config.projectInventoryRelation=event.target.value;markDirty();renderConfig(true);}
    ...
    if(event.target.id==='separateProjectQuota'){const panel=q('#quickProjectQuotaPanel');if(panel)panel.hidden=!event.target.checked;}
    ...
    if(event.target.id==='bookingCutoffUnit'){config.bookingCutoffUnit=event.target.value;markDirty();}
  });
  ...
  document.addEventListener('click',event=>{if(event.target.id==='publishConfig'){saveCurrent(true);localStorage.setItem('scenicPublishedConfig',JSON.stringify(buildVisitorConfig()));currentActivity.status='published';isNewActivity=false;q('#configMode').textContent='编辑预约活动';q('#configSaveState').textContent='已发布 · 刚刚更新';showToast('预约活动已发布，正式游客端已更新');}});
